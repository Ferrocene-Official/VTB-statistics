...
# 接收输入信息
def getinputs():
    global paths, numpath, num1, num2, num3, delta, sigfigure, sigcloud
    paths = os.listdir("input")
    numpath = len(paths)
    num1, num2, num3 = var1.get(), var2.get(), var3.get()
    delta = var4.get()
    sigfigure, sigcloud = sig1.get(), sig2.get()

# 读取直播信息
def liveinformation():
    global records, vtbname, livename
...
checkbutton2 = tk.Checkbutton(window, text="生成词云",
